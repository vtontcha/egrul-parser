...
from config import PAGES_TO_PARSE, SEARCH_QUERY, SEARCH_FILTER


def main():
    print(f"Запуск сбора данных:")
    print(f"  Запрос:   {SEARCH_QUERY}")
    print(f"  Фильтр:   {SEARCH_FILTER or '(нет)'}")
    print(f"  Страниц:  {PAGES_TO_PARSE}")
    print()

    cards = collect_cards(SEARCH_QUERY, PAGES_TO_PARSE, SEARCH_FILTER)

    if cards:
        filepath = save_to_json(cards)
        print(f"\n✅ Готово! Сохранено {len(cards)} карточек:\n   {filepath}")
    else:
        print("\n⚠ Карточки не собраны. Проверьте API-ключ и параметры в config.py.")
